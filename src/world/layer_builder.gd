...
extends Node3D
## Hand-authored greybox deck, assembled from a layout table at load time.
##
## The layout lives in data (ROOMS / CORRIDORS / LIGHTS / PROPS) rather than in a
## 2000-line .tscn so it stays diffable and tweakable while we tune the feel.
## When M2 lands procedural decks, this same box/wall vocabulary becomes the
...
const ROOMS: Array[Dictionary] = [
	{
		"id": "bay", "min": Vector2(-9.0, -7.0), "max": Vector2(9.0, 7.0), "h": 7.0,
		"doors": [{"wall": "n", "at": 0.0}],
	},
	{
		"id": "hub", "min": Vector2(-6.0, -31.0), "max": Vector2(6.0, -19.0), "h": 4.6,
		"doors": [
			{"wall": "s", "at": 0.0},
			{"wall": "n", "at": 0.0},
			{"wall": "w", "at": -25.0},
	...
	{
		"id": "storage", "min": Vector2(-32.0, -32.0), "max": Vector2(-18.0, -18.0), "h": 4.0,
		"doors": [{"wall": "e", "at": -25.0}],
	},
	{
		"id": "machine", "min": Vector2(18.0, -33.0), "max": Vector2(34.0, -17.0), "h": 5.5,
		"doors": [{"wall": "w", "at": -25.0}],
	},
	{
		"id": "terminal", "min": Vector2(-6.5, -51.0), "max": Vector2(6.5, -41.0), "h": 3.6,
		"doors": [{"wall": "s", "at": 0.0}],
	},
]
...
## a strip only delivers ~25% of its energy 5 m away.
const LIGHT_DECAY: float = 0.85

const LIGHTS: Array[Dictionary] = [
	# Docking bay — the only room with real light left.
	{"pos": Vector3(-8.6, 4.2, -2.0), "axis": "x", "len": 4.0,
		"color": Color(0.62, 0.78, 1.0), "energy": 4.0, "range": 20.0, "flicker": false},
	{"pos": Vector3(8.6, 4.2, 2.0), "axis": "x", "len": 4.0,
		"color": Color(0.62, 0.78, 1.0), "energy": 4.0, "range": 20.0, "flicker": false},
	{"pos": Vector3(-8.6, 4.2, 5.0), "axis": "x", "len": 2.4,
		"color": Color(0.55, 0.72, 1.0), "energy": 2.2, "range": 14.0, "flicker": true},
	# Corridor A — one failing strip, then dark.
	{"pos": Vector3(-1.6, 2.9, -12.0), "axis": "x", "len": 2.6,
		"color": Color(0.7, 0.82, 1.0), "energy": 2.6, "range": 13.0, "flicker": true},
	# Junction hub.
	{"pos": Vector3(-5.6, 3.6, -22.0), "axis": "x", "len": 3.0,
		"color": Color(0.6, 0.76, 1.0), "energy": 2.8, "range": 15.0, "flicker": false},
	{"pos": Vector3(5.6, 3.6, -28.0), "axis": "x", "len": 3.0,
		"color": Color(0.6, 0.76, 1.0), "energy": 1.8, "range": 14.0, "flicker": true},
	# Storage hold — sodium, half dead.
	{"pos": Vector3(-25.0, 3.2, -31.6), "axis": "z", "len": 3.4,
		"color": Color(1.0, 0.72, 0.35), "energy": 2.6, "range": 15.0, "flicker": false},
	# Corridor E.
	{"pos": Vector3(12.0, 2.9, -26.6), "axis": "z", "len": 2.6,
		"color": Color(0.65, 0.78, 1.0), "energy": 2.0, "range": 12.0, "flicker": false},
	# Machine room — warm reactor spill.
	{"pos": Vector3(33.6, 4.4, -25.0), "axis": "z", "len": 4.5,
		"color": Color(1.0, 0.62, 0.3), "energy": 3.4, "range": 18.0, "flicker": false},
	{"pos": Vector3(18.4, 4.4, -20.0), "axis": "z", "len": 2.5,
		"color": Color(0.7, 0.8, 1.0), "energy": 1.6, "range": 12.0, "flicker": true},
	# Terminal room — nearly nothing. The screen is the light source.
	{"pos": Vector3(-6.1, 2.8, -47.0), "axis": "z", "len": 2.0,
		"color": Color(0.6, 0.75, 1.0), "energy": 1.1, "range": 10.0, "flicker": false},
]

## Crew drop points in the docking bay. All face north, into the deck.
...
func build() -> void:
	...
	# curve into a featureless white blob once glow is applied.
	_trim_material = _make_emissive(Color(0.55, 0.78, 1.0), 0.85)
	_panel_material = _make_emissive(Color(0.35, 0.62, 1.0), 0.38)
	...
	for light: Dictionary in LIGHTS:
		_build_strip_light(light)

	_build_docking_bay_dressing()
	_build_storage_dressing()
	_build_machine_dressing()
	_build_terminal_dressing()
...
func _build_room(room: Dictionary) -> void:
	...
	_slab(lo, hi, h + SLAB_THICKNESS * 0.5, MAT_CONDUIT)
	...
	# Floor panel lines: a faint grid that gives the eye something to track when
	# the flashlight sweeps across an otherwise featureless slab.
	var inset: float = 1.6
	_panel_line(Vector3(lo.x + inset, 0.012, (lo.y + hi.y) * 0.5),
			Vector3(hi.x - inset, 0.012, (lo.y + hi.y) * 0.5))
	_panel_line(Vector3((lo.x + hi.x) * 0.5, 0.012, lo.y + inset),
			Vector3((lo.x + hi.x) * 0.5, 0.012, hi.y - inset))
...
func _build_corridor(corridor: Dictionary) -> void:
	...
	_slab(lo, hi, -SLAB_THICKNESS * 0.5, MAT_CONDUIT)
	_slab(lo, hi, h + SLAB_THICKNESS * 0.5, MAT_CONDUIT)

	if String(corridor["axis"]) == "z":
		_wall_at_x(lo.x, lo.y, hi.y, h, [])
		_wall_at_x(hi.x, lo.y, hi.y, h, [])
		_panel_line(Vector3((lo.x + hi.x) * 0.5, 0.012, lo.y + 0.6),
				Vector3((lo.x + hi.x) * 0.5, 0.012, hi.y - 0.6))
		_rib_run(corridor, "z")
	else:
		_wall_at_z(lo.y, lo.x, hi.x, h, [])
		_wall_at_z(hi.y, lo.x, hi.x, h, [])
		_panel_line(Vector3(lo.x + 0.6, 0.012, (lo.y + hi.y) * 0.5),
				Vector3(hi.x - 0.6, 0.012, (lo.y + hi.y) * 0.5))
		_rib_run(corridor, "x")


## Structural ribs every few metres. Cheap, but they give a corridor a rhythm
## and a reason for the flashlight to throw moving shadows as you walk.
...
func _wall_at_z(z: float, x0: float, x1: float, h: float, doors: Array) -> void:
	...
	for door: Dictionary in doors:
		...
		_door_frame(Vector3(at, 0.0, z), w, dh, "z")
...
func _wall_at_x(x: float, z0: float, z1: float, h: float, doors: Array) -> void:
	...
	for door: Dictionary in doors:
		...
		_door_frame(Vector3(x, 0.0, at), w, dh, "x")
...
## Emissive jamb + lintel. Doorways are the only reliably lit thing on the deck,
## which makes them read as navigation beacons down a dark corridor.
func _door_frame(base: Vector3, width: float, height: float, normal_axis: String) -> void:
	var half: float = width * 0.5
	var bar: float = 0.09
	if normal_axis == "z":
		_mesh_box(base + Vector3(-half, height * 0.5, 0.0),
				Vector3(bar, height, WALL_THICKNESS + 0.06), _trim_material)
		_mesh_box(base + Vector3(half, height * 0.5, 0.0),
				Vector3(bar, height, WALL_THICKNESS + 0.06), _trim_material)
		_mesh_box(base + Vector3(0.0, height, 0.0),
				Vector3(width, bar, WALL_THICKNESS + 0.06), _trim_material)
	else:
		_mesh_box(base + Vector3(0.0, height * 0.5, -half),
				Vector3(WALL_THICKNESS + 0.06, height, bar), _trim_material)
		_mesh_box(base + Vector3(0.0, height * 0.5, half),
				Vector3(WALL_THICKNESS + 0.06, height, bar), _trim_material)
		_mesh_box(base + Vector3(0.0, height, 0.0),
				Vector3(WALL_THICKNESS + 0.06, bar, width), _trim_material)


func _panel_line(from: Vector3, to: Vector3) -> void:
	var mid: Vector3 = (from + to) * 0.5
	var delta: Vector3 = to - from
	var size: Vector3 = Vector3(maxf(absf(delta.x), 0.07), 0.02, maxf(absf(delta.z), 0.07))
	_mesh_box(mid, size, _panel_material)


# ------------------------------------------------------------------- lights --

func _build_strip_light(spec: Dictionary) -> void:
	...
	var housing_size: Vector3 = Vector3(0.16, 0.18, length) if String(spec["axis"]) == "z" \
			else Vector3(length, 0.18, 0.16)
	...
	light.shadow_enabled = false  # strips are fill light; the flashlight casts.
	_fixtures.add_child(light)


func _build_alarm(pos: Vector3) -> void:
	var alarm: Node3D = Node3D.new()
	alarm.name = "ScanSweep"
	alarm.set_script(load("res://src/world/scan_sweep.gd"))
	alarm.position = pos
	_fixtures.add_child(alarm)

	var housing: MeshInstance3D = MeshInstance3D.new()
	var housing_mesh: BoxMesh = BoxMesh.new()
	housing_mesh.size = Vector3(0.5, 0.3, 0.5)
	housing.mesh = housing_mesh
	housing.material_override = MAT_CONDUIT
	alarm.add_child(housing)

	var dome: MeshInstance3D = MeshInstance3D.new()
	var dome_mesh: SphereMesh = SphereMesh.new()
	dome_mesh.radius = 0.26
	dome_mesh.height = 0.52
	dome_mesh.radial_segments = 12
	dome_mesh.rings = 6
	dome.mesh = dome_mesh
	dome.position = Vector3(0.0, -0.2, 0.0)
	dome.material_override = _make_emissive(Color(1.0, 0.12, 0.12), 2.0)
	alarm.add_child(dome)

	var spot: SpotLight3D = SpotLight3D.new()
	spot.name = "Spot"
	spot.position = Vector3(0.0, -0.25, 0.0)
	spot.rotation = Vector3(deg_to_rad(-52.0), 0.0, 0.0)
	spot.light_color = Color(1.0, 0.13, 0.1)
	...
	alarm.add_child(spot)


# ----------------------------------------------------------------- dressing --

func _build_docking_bay_dressing() -> void:
	_build_alarm(Vector3(0.0, 6.2, 0.0))

	# Blast-door plating on the south wall: this is the breach we dropped through.
	_mesh_box(Vector3(0.0, 3.0, 6.7), Vector3(7.0, 6.0, 0.12), MAT_CONDUIT)
	_mesh_box(Vector3(0.0, 3.0, 6.62), Vector3(0.16, 5.6, 0.08), _trim_material)

	# Gantry running along the west wall.
	...
	_crate(Vector3(6.2, 0.0, -4.4), Vector3(1.5, 1.5, 1.5), 0.18)
	_crate(Vector3(7.0, 1.5, -4.1), Vector3(1.2, 1.2, 1.2), -0.4)
	_crate(Vector3(5.0, 0.0, -5.6), Vector3(1.1, 0.9, 1.4), 0.9)
	_crate(Vector3(-3.4, 0.0, 5.8), Vector3(1.6, 1.2, 1.2), -0.15)
	_crate(Vector3(3.0, 0.0, 6.0), Vector3(1.3, 1.3, 1.3), 0.5)

	_pipe(Vector3(-8.6, 5.4, -6.6), Vector3(-8.6, 5.4, 6.6), 0.22)
	_pipe(Vector3(8.6, 5.8, -6.6), Vector3(8.6, 5.8, 6.6), 0.16)


func _build_storage_dressing() -> void:
	...
	for i: int in 14:
		...
		_crate(Vector3(x, 0.0, z), Vector3(s, s * rng.randf_range(0.7, 1.1), s),
				rng.randf_range(-0.6, 0.6))
		if rng.randf() < 0.4:
			_crate(Vector3(x + rng.randf_range(-0.2, 0.2), s * 0.8, z),
					Vector3(s * 0.8, s * 0.7, s * 0.8), rng.randf_range(-0.8, 0.8))

	_mesh_box(Vector3(-31.6, 2.0, -25.0), Vector3(0.12, 3.2, 8.0), MAT_CONDUIT)
	_pipe(Vector3(-31.5, 3.4, -31.5), Vector3(-31.5, 3.4, -18.5), 0.18)


func _build_machine_dressing() -> void:
	# Reactor stacks: tall silhouettes that break the flashlight into slats.
	for i: int in 4:
		...
		_mesh_box(Vector3(x, 3.28, -29.5), Vector3(1.5, 0.1, 2.0),
				_make_emissive(Color(1.0, 0.5, 0.18), 0.9))
		_pipe(Vector3(x, 3.4, -29.5), Vector3(x, 5.3, -29.5), 0.28)
	...
	for i: int in 5:
		...
		_pipe(Vector3(18.5, 4.6, z), Vector3(33.5, 4.6, z), 0.2)
	_pipe(Vector3(19.0, 0.9, -17.4), Vector3(19.0, 0.9, -32.6), 0.34)
	_pipe(Vector3(19.7, 1.5, -17.4), Vector3(19.7, 1.5, -32.6), 0.22)


func _build_terminal_dressing() -> void:
	# A dead terminal that is not quite dead: the only thing still running down
	# here, and the first place a player will walk toward.
	var base: Vector3 = Vector3(0.0, 0.0, -50.2)
	_box(base + Vector3(0.0, 0.55, 0.0), Vector3(2.4, 1.1, 0.9), MAT_CONDUIT)
	_box(base + Vector3(0.0, 1.55, 0.3), Vector3(2.2, 1.3, 0.3), MAT_CONDUIT)

	var screen: MeshInstance3D = _mesh_box(base + Vector3(0.0, 1.55, 0.12),
			Vector3(1.8, 1.0, 0.06), _make_emissive(Color(0.25, 1.0, 0.45), 0.95))
	screen.name = "TerminalScreen"

	var glow: OmniLight3D = OmniLight3D.new()
	glow.name = "TerminalGlow"
	glow.position = base + Vector3(0.0, 1.6, 0.9)
	...
	_crate(Vector3(-4.6, 0.0, -47.0), Vector3(1.3, 1.0, 1.3), 0.3)
	_crate(Vector3(4.8, 0.0, -49.0), Vector3(1.1, 1.4, 1.1), -0.2)
	_pipe(Vector3(-6.2, 3.1, -50.6), Vector3(-6.2, 3.1, -41.4), 0.16)


# ------------------------------------------------------------------ helpers --

func _crate(pos: Vector3, size: Vector3, yaw: float) -> void:
...
func _pipe(from: Vector3, to: Vector3, radius: float) -> void:
	var delta: Vector3 = to - from
...
## Mesh only — trim, lights housings and decoration the player never bumps into.
